...
! function ($) {
  ...
  $.fn.jmapHeatLabel = function (options) {
    for (var heatmapUl = $("<ul>").addClass("jmap-heatlabel").attr("jmap-uniq", options + "-heatlabel"), index = 0, l = conf.heatmap[options].length; index < l; index++) $("<li>").css({
      display: "inline-block",
      width: "10%",
      height: "30px",
      "background-color": conf.heatmap[options][index]
    }).appendTo(heatmapUl);
    return $(this).append(heatmapUl), this
  }, $.fn.jmapBlackOrWhite = function (_hexcolor) {
    var hexcolor = _hexcolor.toLowerCase();
    if (hexcolor.length <= 4) {
      var hr = hexcolor.substr(1, 1),
        hg = hexcolor.substr(2, 1),
        hb = hexcolor.substr(3, 1);
      hexcolor = "#" + hr + hr + hg + hg + hb + hb
    } else "rgb" == hexcolor.substr(0, 3) ? (hexcolor = hexcolor.replace(" ", ""), hexcolor = hexcolor.replace("rgba(", "["), hexcolor = hexcolor.replace("rgb(", "["), hexcolor = hexcolor.replace(")", "]"), hexArray = eval(hexcolor), hexArray.length >= 4 && hexArray.pop(), hexcolor = "#", $.each(hexArray, (function (index, val) {
      hexcolor += ("0" + hexArray[index].toString(16)).slice(-2)
    }))) : hexcolor.match(/^a-z/gi) && (hexcolor = conf.colorgroups[hexcolor]);
    try {
      var r = parseInt(hexcolor.substr(1, 2), 16),
        g = parseInt(hexcolor.substr(3, 2), 16),
        b = parseInt(hexcolor.substr(5, 2), 16);
      return (299 * r + 587 * g + 114 * b) / 1e3 < 160 ? "#ffffff" : "#000000"
    } catch (e) {
      return _hexcolor
    }
  }, $.fn.jmap = function (options) {
    if ("update" == options) return this;
    identifier++;
    ...
    var containerCss = {
      ...
      containerSelector = 'div[jmap-id="%s1"]'.replace("%s1", uniqClass),
      containerStyle = JSON.stringify(containerCss).replace(/",/g, '";').replace(/"/g, "");
    stylers.push(containerSelector + containerStyle);
    var containerDiv = $("<div>").attr("jmap-id", params.id).addClass(params.containerClass),
      contentCss = {
      ...
      contentSelector = 'div[jmap-id="%s1"] .%s2 '.replace("%s1", uniqClass).replace("%s2", params.contentClass),
      contentStyle = JSON.stringify(contentCss).replace(/",/g, '";').replace(/"/g, "");
    if (stylers.push(contentSelector + contentStyle), "grid" == params.viewType) {
      var row = Math.ceil((conf.prefectures.length + params.gridOffset) / params.gridNumber),
        col = params.gridNumber,
        contentGridCss = {
        ...
        contentGridSelector = 'div[jmap-id="%s1"] .%s2[jmap-view="grid"] '.replace("%s1", uniqClass).replace("%s2", params.contentClass),
        contentGridStyle = JSON.stringify(contentGridCss).replace(/",/g, '";').replace(/"/g, "");
      stylersPrimal.push(contentGridSelector + contentGridStyle), params.showInfobox = !1, params.showHeatlabel = !1, params.showIslandDivider = !1
    }
    var contentDiv = $("<div>").attr("jmap-view", params.viewType).addClass(params.contentClass);
    if (0 != params.skew && contentDiv.attr("jmap-skew", !0), params.showIslandDivider) {
      var divider = conf.divider,
        ...
        dividerSelector = 'div[jmap-id="%s1"] .%s2 '.replace("%s1", uniqClass).replace("%s2", params.dividerClass),
        dividerStyle = JSON.stringify(dividerCss).replace(/",/g, '";').replace(/"/g, "");
      stylers.push(dividerSelector + dividerStyle);
      var dividerDiv = $("<div>").addClass(params.dividerClass).appendTo(contentDiv)
    }
    var heatmapMax = null,
      heatmapMin = null;
    if (params.showHeatmap)
      for (var index in params.areas) {
        ...
        area.number && (heatmapMax ? area.number > heatmapMax && (heatmapMax = area.number) : heatmapMax = area.number, heatmapMin ? area.number < heatmapMin && (heatmapMin = area.number) : heatmapMin = area.number)
      }
    var infoboxDiv = null;
    if (params.showInfobox) {
      var infobox = conf.infobox,
        ...
        infoboxSelector = 'div[jmap-id="%s1"] .%s2 '.replace("%s1", uniqClass).replace("%s2", params.infoboxClass),
        infoboxStyle = JSON.stringify(infoboxCss).replace(/",/g, '";').replace(/"/g, "");
      if (stylers.push(infoboxSelector + infoboxStyle), infoboxDiv = $(this).find(".jmap-infobox"), infoboxDiv.length > 0 ? infoboxDiv.addClass(params.infoboxClass).appendTo(contentDiv) : infoboxDiv = $("<div>").addClass(params.infoboxClass).html(params.infoboxContent).appendTo(contentDiv), params.showHeatmap && params.showHeatlabel) {
        var heatmapCss = {
            padding: "0px",
            margin: "0px",
            "max-width": "250px"
          },
          heatmapSelector = 'div[jmap-id="%s1"] .jmap-heatlabel'.replace("%s1", uniqClass),
          heatmapStyle = JSON.stringify(heatmapCss).replace(/",/g, '";').replace(/"/g, "");
        ...
        var heatmapLabelLiCss = {
          ...
          heatmapLabelLiSelector = 'div[jmap-id="%s1"] .jmap-heatlabel  li '.replace("%s1", uniqClass),
          heatmapLabelLiStyle = JSON.stringify(heatmapLabelLiCss).replace(/",/g, '";').replace(/"/g, "");
        stylers.push(heatmapLabelLiSelector + heatmapLabelLiStyle);
        var heatmapLabelLiHoverCss = {
            opacity: "0.8",
            border: "1px solid #ccc"
          },
          heatmapLabelLiHoverSelector = 'div[jmap-id="%s1"] .jmap-heatlabel  li:hover '.replace("%s1", uniqClass),
          heatmapLabelLiHoverStyle = JSON.stringify(heatmapLabelLiHoverCss).replace(/",/g, '";').replace(/"/g, "");
        stylers.push(heatmapLabelLiHoverSelector + heatmapLabelLiHoverStyle);
        var heatmapLabelLiFirstChildCss = {
            "border-radius": "3px 0 0 3px"
          },
          heatmapLabelLiFirstChildSelector = 'div[jmap-id="%s1"] .jmap-heatlabel li:first-child '.replace("%s1", uniqClass),
          heatmapLabelLiFirstChildStyle = JSON.stringify(heatmapLabelLiFirstChildCss).replace(/",/g, '";').replace(/"/g, "");
        stylers.push(heatmapLabelLiFirstChildSelector + heatmapLabelLiFirstChildStyle);
        var heatmapLabelLiLastChildCss = {
            "border-radius": "0 3px 3px 0"
          },
          heatmapLabelLiLastChildCssSelector = 'div[jmap-id="%s1"] .jmap-heatlabel li:last-child '.replace("%s1", uniqClass),
          heatmapLabelLiLastChildCssStyle = JSON.stringify(heatmapLabelLiLastChildCss).replace(/",/g, '";').replace(/"/g, "");
        ...
        var heatmapLabelSpan = {
          ...
          heatmapLabelSpanCssSelector = 'div[jmap-id="%s1"] .jmap-heatlabel li span '.replace("%s1", uniqClass),
          heatmapLabelSpanCssStyle = JSON.stringify(heatmapLabelSpan).replace(/",/g, '";').replace(/"/g, "");
        stylers.push(heatmapLabelSpanCssSelector + heatmapLabelSpanCssStyle);
        var heatmapLabelSpanHover = {
            display: "block",
            opacity: "1"
          },
          heatmapLabelSpanHoverCssSelector = 'div[jmap-id="%s1"] .jmap-heatlabel li:hover span'.replace("%s1", uniqClass),
          heatmapLabelSpanHoverCssStyle = JSON.stringify(heatmapLabelSpanHover).replace(/",/g, '";').replace(/"/g, "");
        stylers.push(heatmapLabelSpanHoverCssSelector + heatmapLabelSpanHoverCssStyle), $("body").on("mouseover", heatmapLabelLiSelector, (function (e) {
        ...
        for (var heatmapUl = $("<ul>").addClass("jmap-heatlabel"), inc = Math.round((heatmapMax - heatmapMin) / 10), index = 0, l = params.heatmapColors.length; index < l; index++) {
          var value = 0,
            text = "",
            range = "";
          if (params.heatmapConditions[params.heatmapConditions.length - 1 - index]) {
            ...
            str.match(/>=/) && (range = "以上"), str.match(/<=/) && (range = "以下"), "" == range && (str.match(/>/) && (range = "超"), str.match(/</) && (range = "未満"), str.match(/=/) && (range = "等しい")), value = parseInt(str.replace(">=", "").replace("<=", "").replace("<", "").replace("<", "")), text = value.toLocaleString() + params.heatmapLabelUnit + range
          } else range = "以上", value = heatmapMin + inc * index, text = value.toLocaleString() + params.heatmapLabelUnit + range, text += " ", text += (heatmapMin + inc * (index + 1)).toLocaleString() + "未満";
          var span = $("<span>").text(text).data("data", {
              index: index,
              text: text,
              value: value,
              range: range,
              unit: params.heatmapLabelUnit
            }),
            heatmapLabelLiIndivisualCss = {
              "background-color": params.heatmapColors[index]
            },
            heatmapLabelLiIndivisualSelector = 'div[jmap-id="%s1"] .jmap-heatlabel li[jmap-uniq="%s2"]'.replace("%s1", uniqClass).replace("%s2", index),
            heatmapLabelLiIndivisualStyle = JSON.stringify(heatmapLabelLiIndivisualCss).replace(/",/g, '";').replace(/"/g, "");
          stylers.push(heatmapLabelLiIndivisualSelector + heatmapLabelLiIndivisualStyle), $("<li>").attr("jmap-uniq", index).append(span).appendTo(heatmapUl)
        }
    ...
    params.showTextShadow && (prefectureCss["text-shadow"] = "0 0 %s1 %s2".replace("%s1", params.textShadowWidth).replace("%s2", params.textShadowColor));
    var prefectureSelector = 'div[jmap-id="%s1"] .%s2 '.replace("%s1", uniqClass).replace("%s2", params.prefectureClass),
      prefectureStyle = JSON.stringify(prefectureCss).replace(/",/g, '";').replace(/"/g, "");
    if (stylers.push(prefectureSelector + prefectureStyle), $("body").on("click", prefectureSelector, (function (e) {
      ...
      var prefectureHoverCss = {
        ...
        prefectureHoverSelector = 'div[jmap-id="%s1"] .%s2:hover '.replace("%s1", uniqClass).replace("%s2", params.prefectureClass),
        prefectureHoverStyle = JSON.stringify(prefectureHoverCss).replace(/",/g, '";').replace(/"/g, "");
      stylersPrimal.push(prefectureHoverSelector + prefectureHoverStyle)
    }
    if (params.showTextNoneSkewed && 0 != params.skew) {
      var prefectureInnerDivCss = {
          transform: "skew(%d1deg)".replace("%d1", params.skew)
        },
        prefectureInnerDivSelector = 'div[jmap-id="%s1"] .%s2 div'.replace("%s1", uniqClass).replace("%s2", params.prefectureClass),
        prefectureInnerDivStyle = JSON.stringify(prefectureInnerDivCss).replace(/"/g, "").replace(/,/g, ";");
      stylers.push(prefectureInnerDivSelector + prefectureInnerDivStyle)
    }
    var gridRowIndex = 1,
      gridColIndex = params.gridOffset;
    $.each(conf.prefectures, (function (_index, pref) {
      var pref = $.extend({
          option: {}
        }, pref),
        option = params.areas.filter((function (_pref) {
          return pref.code == _pref.code
        }))[0] || {};
      for (var index in pref.option = $.extend(pref.option, option), conf.area8)
      ...
      params.showRoundedPrefecture && (prefectureIndivisualCss = $.extend(prefectureIndivisualCss, {
        "border-radius": pref.radius.join(" ").replace(/1/g, params.prefectureRadius)
      })), option.fontColor && (prefectureIndivisualCss.color = option.fontColor);
      var prefectureIndivisualSelector = 'div[jmap-id="%s1"] .%s2[jmap-uniq="%s3"] '.replace("%s1", uniqClass).replace("%s2", params.prefectureClass).replace("%s3", pref.code),
        prefectureIndivisualStyle = JSON.stringify(prefectureIndivisualCss).replace(/"/g, "").replace(/,/g, ";");
      if (stylers.push(prefectureIndivisualSelector + prefectureIndivisualStyle), "grid" == params.viewType) {
        var prefectureIndivisualGridCss = {
          ...
          prefectureIndivisualGridSelector = '%s1 .%s2[jmap-uniq="%s3"] '.replace("%s1", contentGridSelector).replace("%s2", params.prefectureClass).replace("%s3", pref.code),
          prefectureIndivisualGridStyle = JSON.stringify(prefectureIndivisualGridCss).replace(/"/g, "").replace(/,/g, ";");
        stylersPrimal.push(prefectureIndivisualGridSelector + prefectureIndivisualGridStyle), gridColIndex >= params.gridNumber && (gridColIndex = 0, gridRowIndex += 1), gridColIndex++
      }
      if (params.selectable)
        if (option.hoverColor) {
          var prefectureIndivisualHoverCss = {
            ...
            prefectureIndivisualHoverSelector = 'div[jmap-id="%s1"] .%s2[jmap-uniq="%s3"]:hover '.replace("%s1", uniqClass).replace("%s2", params.prefectureClass).replace("%s3", pref.code),
            prefectureIndivisualHoverStyle = JSON.stringify(prefectureIndivisualHoverCss).replace(/"/g, "").replace(/,/g, ";");
        ...
        var css = {
          ...
          selector = 'div[jmap-id="%s1"] .%s2[jmap-uniq="%s3"]:hover '.replace("%s1", uniqClass).replace("%s2", params.prefectureClass).replace("%s3", pref.code),
          style = JSON.stringify(css).replace(/"/g, "").replace(/,/g, ";");
        stylersPrimal.push(selector + style)
      }
      var prefDiv = $("<div>").data("data", pref).addClass(params.prefectureClass).attr("jmap-uniq", pref.code);
      if (params.showPrefectureName && !option.name) switch (params.prefectureNameType) {
      case "full":
      ...
      } else params.showPrefectureName && option.name && prefDiv.html(option.name);
      if (params.showTextNoneSkewed && 0 != params.skew) {
        var textDiv = $("<div>").html(prefDiv.html());
        prefDiv.html(textDiv)
      }
      contentDiv.append(prefDiv), setTimeout((function () {
        params.onLoad.call(prefDiv, null, pref)
      }))
    }));
    var jmapStyle = $("<style>").attr("type", "text/css").html(stylers.join("\n") + "\n" + stylersPrimal.join("\n"));
    if (containerDiv.append(contentDiv), $(this).empty().append(jmapStyle).append(containerDiv), 0 != params.skew) {
      var increasedWidth = (contentDiv.height() / Math.tan((90 - params.skew) * (Math.PI / 180)) / 2).toFixed(3),
        conteSkewCss = {
          padding: "0 " + increasedWidth + "px",
          transform: "skew(-%d1deg)".replace("%d1", params.skew)
        },
        contentSkewSelector = 'div[jmap-id="%s1"] .%s2[jmap-skew="true"] '.replace("%s1", uniqClass).replace("%s2", params.contentClass),
        contentSkewStyle = JSON.stringify(conteSkewCss).replace(/",/g, '";').replace(/"/g, "");
      if (stylersPrimal.push(contentSkewSelector + contentSkewStyle), jmapStyle.append(contentSkewSelector + contentSkewStyle + "\n"), infoboxDiv) {
        var infoboxSkewCss = {
            "margin-left": "-%d1".replace("%d1", increasedWidth / 2 * 3 + "px"),
            transform: "skew(%d1deg)".replace("%d1", params.skew)
          },
          infoboxSkewSelector = "%s1 .%s2".replace("%s1", contentSkewSelector).replace("%s2", params.infoboxClass),
          infoboxSkewStyle = JSON.stringify(infoboxSkewCss).replace(/",/g, '";').replace(/"/g, "");
        stylersPrimal.push(infoboxSkewSelector + infoboxSkewStyle), jmapStyle.append(infoboxSkewSelector + infoboxSkewStyle + "\n")
      }
    }
    return this
  }
